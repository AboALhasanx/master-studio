...
> **نص الملزمة:** "Nondependency-Oriented Data → Data where values are independent of each other."
>
> **نص الملزمة:** "Dependency-Oriented Data → Data where values are related or dependent on each other, often involving order, time, or structure."
...
> **نص الملزمة:** "Definition: Data represented by multiple numerical attributes."
>
> **نص الملزمة:** "Characteristics: Continuous values (real numbers). Can be represented in a vector form. Use cases: Regression, clustering, anomaly detection."
...
> 
> > **نص الملزمة:** "Categorical Data: Attributes represent discrete categories/labels."
> >
> > **نص الملزمة:** "Mixed Attribute Data: Contains both numerical and categorical variables."
> >
> > **نص الملزمة:** "Challenges: Need special encoding techniques (e.g., one-hot encoding). Distance metrics are harder to define compared to numerical data."
> 
...
> 
> > **نص الملزمة:** "Binary Data: Attributes with only two possible values {0,1} or {True, False}."
> >
> > **نص الملزمة:** "Set Data: Each object described by a set of items."
> >
> > **نص الملزمة:** "Applications: Association rule mining (e.g., Market Basket Analysis). Recommendation systems."
> 
...
> 
> > **نص الملزمة:** "Definition: Data stored as unstructured textual information."
> >
> > **نص الملزمة:** "Text is unstructured and must be converted into numerical form (Bag of Words, TF-IDF, word embeddings)."
> >
> > **نص الملزمة:** "Applications: Sentiment analysis, topic modeling, chatbots, search engines."
> 
...
> > **نص الملزمة:** "Represents text as a vector of word counts."
> > **نص الملزمة:** "Vocabulary: [cat, dog, mat, log, sat, the, on] — Sentence 1 → [1, 0, 1, 0, 1, 2, 1]"
> - **شلون يشتغل (مثال):**
...
> > **نص الملزمة:** "TF-IDF(t,d) = TF(t,d) × IDF(t)"
> > **نص الملزمة:** "Better than BoW because it reduces the impact of frequent but uninformative words."
...
> > **نص الملزمة:** "Advantage: Captures semantic relationships ('king – man + woman ≈ queen')."
...
> **نص الملزمة:** "Definition: A sequence of values recorded over time."
>
> **نص الملزمة:** "Characteristics: Temporal dependency (past values influence future values). Often analyzed using ARIMA, LSTMs, Transformers. Applications: Forecasting, anomaly detection."
...
> 
> > **نص الملزمة:** "Definition: Data represented as sequences of discrete symbols."
> >
> > **نص الملزمة:** "Challenges: Pattern matching, alignment, and subsequence analysis."
> 
...
> 
> > **نص الملزمة:** "Definition: Data representing objects in a spatial or geographical context."
> >
> > **نص الملزمة:** "Challenges: Spatial autocorrelation (nearby objects are more related)."
> 
...
> 
> > **نص الملزمة:** "Definition: Data represented as nodes and edges."
> >
> > **نص الملزمة:** "Characteristics: Strongly structured data. Requires graph algorithms (PageRank, community detection, Graph Neural Networks)."
> 
...
> **نص الملزمة:** "Data Collection – Gathering data from multiple sources (databases, files, APIs, sensors, etc.)."
> **نص الملزمة:** "Data Cleaning – Handling missing values, removing duplicates, correcting errors, and resolving inconsistencies."
> **نص الملزمة:** "Data Integration – Combining data from different sources into a single coherent dataset."
> **نص الملزمة:** "Data Transformation – Converting data into a consistent format (e.g., encoding categorical variables, normalization, scaling)."
> **نص الملزمة:** "Data Reduction – Reducing the size/dimensionality of data by removing irrelevant features or aggregating values."
> **نص الملزمة:** "Data Splitting – Dividing data into training, validation, and test sets (in machine learning tasks)."
...
> **نص الملزمة:** "Ensures accuracy (removes errors and inconsistencies). Improves efficiency (organized data is faster to process). Enhances model performance (clean, balanced, and relevant data leads to better predictions). Supports better decision-making (reliable insights come from well-prepared data)."

---

## القسم 5 — استخراج السمات (Feature Extraction)

> **نص الملزمة:** "Feature extraction is the process of creating new features from existing data to represent its important characteristics in a form suitable for data mining or machine learning algorithms."
>
> **نص الملزمة:** "Instead of giving the complete image directly to a traditional algorithm, we can represent it using useful features."
...
> **نص الملزمة:** "Data Type Portability means converting data from one representation or type into another representation so that it can be processed by a data mining algorithm."
...
> **نص الملزمة:** "Discretization is the process of converting continuous numeric data into discrete categorical data by dividing the range of values into intervals (bins)."
>
> **نص الملزمة:** "Example: Age = [15, 18, 22, 35, 42, 50, 63, 70]. Equal-Width Binning (width=20): Categories: Teen (0–20), Young (21–40), Middle-aged (41–60), Senior (61–80). Result: [Teen, Teen, Young, Young, Middle-aged, Middle-aged, Senior, Senior]."
...
> **نص الملزمة:** "Why Not Simply Use A = 1, B = 2, O = 3? Using A = 1, B = 2, O = 3 may incorrectly suggest that the categories have a numerical order. For nominal categories such as blood type, there is no meaningful order. Therefore, one-hot encoding is usually more appropriate for nominal categorical data."

> **نص الملزمة:** "One-Hot Encoding: Each category gets a new binary column. Example: Color = {Red, Blue, Green} → Red 1 0 0, Blue 0 1 0, Green 0 0 1."
...
> **نص الملزمة:** "Instead of keeping raw heart rate values [75, 77, 80, 60, 58, 90], we can convert them into symbols like: [N, N, H, L, L, H] (where N = Normal, H = High, L = Low)."

> **نص الملزمة:** "SAX Steps: Normalize the series (mean = 0, variance = 1). Divide series into equal-sized segments. Compute average value for each segment. Map averages into symbols using breakpoints."
...
> **نص الملزمة:** "A graph G = (V, E) consists of nodes (V) and edges (E) that connect them."

طرق:

- **Node-Level:** درجة العقدة (degree)، clustering coefficient، centrality (betweenness/closeness/eigenvector)، **PageRank**. مثال الدكتور:
> **نص الملزمة:** "Example: Social network node → [degree=5, clustering=0.3, betweenness=0.12]"
...
> **نص الملزمة:** "Node2Vec / DeepWalk: Random walks on the graph → treat as sentences → use Word2Vec to get node embeddings."
> **نص الملزمة:** "Graph Neural Networks (GNNs): Aggregate features from neighbors → node-level or graph-level embeddings. Examples: GCN, GraphSAGE, GAT."
...
> **نص الملزمة:** "Instead of converting everything into numerical data, relationships between objects can be represented using a graph."
>
> **نص الملزمة:** "Suppose we have five patients. If two patients are sufficiently similar, we connect them. The nodes represent objects, and the edges represent similarity."
>
> **نص الملزمة:** "A similarity graph can be created using a similarity or distance measure. For example: If distance(P1, P2) < threshold → Connect P1 and P2. The graph represents which objects are similar to each other."
>
> **نص الملزمة:** "Applications: Clustering. Classification. Nearest-neighbor analysis. Outlier detection."
...
> **نص الملزمة:** "Data type conversion is useful, but it may result in information loss. Example: Original ages: 21, 22, 39. After discretization: Adult, Adult, Adult. The exact ages have disappeared. Therefore: A good conversion should preserve the important information needed for the mining task."
...
> **نص الملزمة:** "MCAR – Missing Completely at Random: The missingness has no relationship with the data (observed or unobserved)."
> **نص الملزمة:** "MAR – Missing at Random: The missingness depends on observed data but not on the missing data itself."
> **نص الملزمة:** "MNAR – Missing Not at Random: The missingness depends on the missing data itself."
...
**الاستراتيجيات:**
> **نص الملزمة:** "Listwise Deletion: Remove any record (row) with missing values. Column Deletion: Remove attributes (columns) with too many missing values. Useful if >50% values are missing in a column."
...
> **نص الملزمة:** "Min-Max Scaling: Rescales values to a fixed range, usually [0, 1]. Formula: x' = (x − x_min)/(x_max − x_min)."
> **نص الملزمة:** "Z-Score Standardization: Centers the data at 0 and scales to unit variance. Formula: x' = (x − μ)/σ."
...
> **نص الملزمة:** "Sampling: Selecting a subset of data to represent the entire dataset."
> **نص الملزمة:** "Filter methods: Evaluate features using statistical measures (correlation, chi-square). Wrapper methods: Use ML algorithms to test feature subsets (e.g., recursive feature elimination). Embedded methods: Feature selection is part of model training (e.g., Lasso regression)."
...
> **نص الملزمة:** "PCA Steps: Standardize the data (mean = 0, variance = 1). Compute the covariance matrix. Compute eigenvectors and eigenvalues. Sort eigenvectors by eigenvalues → choose top-k components. Project data onto these principal components."
  - **SVD:** `A = U Σ Vᵀ`، يقلل الأبعاد. أساس LSA بالنصوص.
> **نص الملزمة:** "SVD Factorizes the data matrix A into three matrices: A = U Σ Vᵀ."
