# قراءة بحثية عميقة — محاضرة 2 و 3: أنواع البيانات وقابلية النقل (النسخة v3)

> **ملاحظة مهمة قبل تبدأ:** الدكتور ما يعلم — هو **يوزع بحث**. في هالنسخة v3، كل اقتباس إنكليزي داخل صندوق `Verbatim` هو **حرفياً من كلام الدكتور** بالمصادر (ملفات الـ docx الأصلية) — ما انجم من راسي. تحته مباشرة الشرح العربي الفهمي. باقي الملزمة كله عربي (والمصطلحات الإنكليزية كما هي بالمحاضرة).
>
...
> **Verbatim (Doctor — Week 02):** "Nondependency-Oriented Data → Data where values are independent of each other."
>
> **Verbatim (Doctor — Week 02):** "Dependency-Oriented Data → Data where values are related or dependent on each other, often involving order, time, or structure."
...
> **Verbatim (Doctor — Week 02):** "Definition: Data represented by multiple numerical attributes."
>
> **Verbatim (Doctor — Week 02):** "Characteristics: Continuous values (real numbers). Can be represented in a vector form. Use cases: Regression, clustering, anomaly detection."
...
> **Verbatim (Doctor — Week 02):** "Categorical Data: Attributes represent discrete categories/labels."
>
> **Verbatim (Doctor — Week 02):** "Mixed Attribute Data: Contains both numerical and categorical variables."
>
> **Verbatim (Doctor — Week 02):** "Challenges: Need special encoding techniques (e.g., one-hot encoding). Distance metrics are harder to define compared to numerical data."
...
> **Verbatim (Doctor — Week 02):** "Binary Data: Attributes with only two possible values {0,1} or {True, False}."
>
> **Verbatim (Doctor — Week 02):** "Set Data: Each object described by a set of items."
>
> **Verbatim (Doctor — Week 02):** "Applications: Association rule mining (e.g., Market Basket Analysis). Recommendation systems."
...
> **Verbatim (Doctor — Week 02):** "Definition: Data stored as unstructured textual information."
>
> **Verbatim (Doctor — Week 02):** "Text is unstructured and must be converted into numerical form (Bag of Words, TF-IDF, word embeddings)."
>
> **Verbatim (Doctor — Week 02):** "Applications: Sentiment analysis, topic modeling, chatbots, search engines."
...
> **Verbatim (Doctor — Week 02):** "Represents text as a vector of word counts."
> **Verbatim (Doctor — Week 02):** "Vocabulary: [cat, dog, mat, log, sat, the, on] — Sentence 1 → [1, 0, 1, 0, 1, 2, 1]"
- **شلون يشتغل (مثال):**
...
#### بطاقة بحث: TF-IDF (Term Frequency – Inverse Document Frequency) — ★★ اللي سألت عنه
...
> **Verbatim (Doctor — Week 02):** "TF-IDF(t,d) = TF(t,d) × IDF(t)"
> **Verbatim (Doctor — Week 02):** "Better than BoW because it reduces the impact of frequent but uninformative words."
...
> **Verbatim (Doctor — Week 02):** "Advantage: Captures semantic relationships ('king – man + woman ≈ queen')."
...
> **Verbatim (Doctor — Week 02):** "Definition: A sequence of values recorded over time."
>
> **Verbatim (Doctor — Week 02):** "Characteristics: Temporal dependency (past values influence future values). Often analyzed using ARIMA, LSTMs, Transformers. Applications: Forecasting, anomaly detection."
...
> **Verbatim (Doctor — Week 02):** "Definition: Data represented as sequences of discrete symbols."
>
> **Verbatim (Doctor — Week 02):** "Challenges: Pattern matching, alignment, and subsequence analysis."
...
> **Verbatim (Doctor — Week 02):** "Definition: Data representing objects in a spatial or geographical context."
>
> **Verbatim (Doctor — Week 02):** "Challenges: Spatial autocorrelation (nearby objects are more related)."
...
> **Verbatim (Doctor — Week 02):** "Definition: Data represented as nodes and edges."
>
> **Verbatim (Doctor — Week 02):** "Characteristics: Strongly structured data. Requires graph algorithms (PageRank, community detection, Graph Neural Networks)."
...
> **Verbatim (Doctor — Week 02):** "Data Collection – Gathering data from multiple sources (databases, files, APIs, sensors, etc.)."
> **Verbatim (Doctor — Week 02):** "Data Cleaning – Handling missing values, removing duplicates, correcting errors, and resolving inconsistencies."
> **Verbatim (Doctor — Week 02):** "Data Integration – Combining data from different sources into a single coherent dataset."
> **Verbatim (Doctor — Week 02):** "Data Transformation – Converting data into a consistent format (e.g., encoding categorical variables, normalization, scaling)."
> **Verbatim (Doctor — Week 02):** "Data Reduction – Reducing the size/dimensionality of data by removing irrelevant features or aggregating values."
> **Verbatim (Doctor — Week 02):** "Data Splitting – Dividing data into training, validation, and test sets (in machine learning tasks)."
...
> **Verbatim (Doctor — Week 02):** "Ensures accuracy (removes errors and inconsistencies). Improves efficiency (organized data is faster to process). Enhances model performance (clean, balanced, and relevant data leads to better predictions). Supports better decision-making (reliable insights come from well-prepared data)."

---

## القسم 5 — استخراج السمات (Feature Extraction)

> **Verbatim (Doctor — Week 03):** "Feature extraction is the process of creating new features from existing data to represent its important characteristics in a form suitable for data mining or machine learning algorithms."
>
> **Verbatim (Doctor — Week 03):** "Instead of giving the complete image directly to a traditional algorithm, we can represent it using useful features."
...
> **Verbatim (Doctor — Week 03):** "Data Type Portability means converting data from one representation or type into another representation so that it can be processed by a data mining algorithm."
...
> **Verbatim (Doctor — Week 02):** "Discretization is the process of converting continuous numeric data into discrete categorical data by dividing the range of values into intervals (bins)."
>
> **Verbatim (Doctor — Week 02):** "Example: Age = [15, 18, 22, 35, 42, 50, 63, 70]. Equal-Width Binning (width=20): Categories: Teen (0–20), Young (21–40), Middle-aged (41–60), Senior (61–80). Result: [Teen, Teen, Young, Young, Middle-aged, Middle-aged, Senior, Senior]."
...
> **Verbatim (Doctor — Week 03):** "Why Not Simply Use A = 1, B = 2, O = 3? Using A = 1, B = 2, O = 3 may incorrectly suggest that the categories have a numerical order. For nominal categories such as blood type, there is no meaningful order. Therefore, one-hot encoding is usually more appropriate for nominal categorical data."

> **Verbatim (Doctor — Week 02):** "One-Hot Encoding: Each category gets a new binary column. Example: Color = {Red, Blue, Green} → Red 1 0 0, Blue 0 1 0, Green 0 0 1."
...
> **Verbatim (Doctor — Week 02):** "Instead of keeping raw heart rate values [75, 77, 80, 60, 58, 90], we can convert them into symbols like: [N, N, H, L, L, H] (where N = Normal, H = High, L = Low)."

> **Verbatim (Doctor — Week 02):** "SAX Steps: Normalize the series (mean = 0, variance = 1). Divide series into equal-sized segments. Compute average value for each segment. Map averages into symbols using breakpoints."
...
> **Verbatim (Doctor — Week 02):** "A graph G = (V, E) consists of nodes (V) and edges (E) that connect them."

طرق:

- **Node-Level:** درجة العقدة (degree)، clustering coefficient، centrality (betweenness/closeness/eigenvector)، **PageRank**. مثال الدكتور:
> **Verbatim (Doctor — Week 02):** "Example: Social network node → [degree=5, clustering=0.3, betweenness=0.12]"
...
> **Verbatim (Doctor — Week 02):** "Density = 2*|E| / (|V|*(|V|-1))"
...
> **Verbatim (Doctor — Week 02):** "Node2Vec / DeepWalk: Random walks on the graph → treat as sentences → use Word2Vec to get node embeddings."
> **Verbatim (Doctor — Week 02):** "Graph Neural Networks (GNNs): Aggregate features from neighbors → node-level or graph-level embeddings. Examples: GCN, GraphSAGE, GAT."
...
> **Verbatim (Doctor — Week 03):** "Instead of converting everything into numerical data, relationships between objects can be represented using a graph."
>
> **Verbatim (Doctor — Week 03):** "Suppose we have five patients. If two patients are sufficiently similar, we connect them. The nodes represent objects, and the edges represent similarity."
>
> **Verbatim (Doctor — Week 03):** "A similarity graph can be created using a similarity or distance measure. For example: If distance(P1, P2) < threshold → Connect P1 and P2. The graph represents which objects are similar to each other."
>
> **Verbatim (Doctor — Week 03):** "Applications: Clustering. Classification. Nearest-neighbor analysis. Outlier detection."
...
> **Verbatim (Doctor — Week 03):** "Data type conversion is useful, but it may result in information loss. Example: Original ages: 21, 22, 39. After discretization: Adult, Adult, Adult. The exact ages have disappeared. Therefore: A good conversion should preserve the important information needed for the mining task."
...
> **Verbatim (Doctor — Week 02):** "MCAR – Missing Completely at Random: The missingness has no relationship with the data (observed or unobserved)."
> **Verbatim (Doctor — Week 02):** "MAR – Missing at Random: The missingness depends on observed data but not on the missing data itself."
> **Verbatim (Doctor — Week 02):** "MNAR – Missing Not at Random: The missingness depends on the missing data itself."
...
**الاستراتيجيات:**
> **Verbatim (Doctor — Week 02):** "Listwise Deletion: Remove any record (row) with missing values. Column Deletion: Remove attributes (columns) with too many missing values. Useful if >50% values are missing in a column."
...
> **Verbatim (Doctor — Week 02):** "Min-Max Scaling: Rescales values to a fixed range, usually [0, 1]. Formula: x' = (x − x_min)/(x_max − x_min)."
> **Verbatim (Doctor — Week 02):** "Z-Score Standardization: Centers the data at 0 and scales to unit variance. Formula: x' = (x − μ)/σ."
...
> **Verbatim (Doctor — Week 02):** "Sampling: Selecting a subset of data to represent the entire dataset."
> **Verbatim (Doctor — Week 02):** "Filter methods: Evaluate features using statistical measures (correlation, chi-square). Wrapper methods: Use ML algorithms to test feature subsets (e.g., recursive feature elimination). Embedded methods: Feature selection is part of model training (e.g., Lasso regression)."
...
> **Verbatim (Doctor — Week 02):** "PCA Steps: Standardize the data (mean = 0, variance = 1). Compute the covariance matrix. Compute eigenvectors and eigenvalues. Sort eigenvectors by eigenvalues → choose top-k components. Project data onto these principal components."
  - **SVD:** `A = U Σ Vᵀ`، يقلل الأبعاد. أساس LSA بالنصوص.
> **Verbatim (Doctor — Week 02):** "SVD Factorizes the data matrix A into three matrices: A = U Σ Vᵀ."
