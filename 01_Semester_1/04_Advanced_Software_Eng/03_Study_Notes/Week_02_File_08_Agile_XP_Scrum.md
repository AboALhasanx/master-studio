...
> **Subject · Week · File:** 04_Advanced_Software_Eng · Week 02 · File 8 of 10 · Agile, XP and Scrum
> **Sources:** Sommerville §3.1 p.74–77 (context, agile definition, the four manifesto statements, where it works/fails); Mall §2.4–§2.4.4 p.105–114 (agile overview, agile-vs-other-models, Extreme Programming, Scrum).

---

## WHERE THIS SITS

**File 07 (Unified Process)** closed on its own weight: UP is **disciplined and complete — and heavy**. For a small team with fast-changing requirements, even UP is **too much process** (Sommerville p.70: "not a suitable process for all types of development, e.g., embedded software").

**This file answers:** what if the problem is not *which process* but the *assumption that we can plan at all*? Agile throws out the heavyweight plan-driven machinery and replaces it with **people, working software, and the acceptance that requirements will change**. [Foundational Knowledge / Standard Concept]
...
**AR.** السبب الجذري: بالبيئة السريعة التغيّر، **مستحيل تستخرج مطلوبات مستقرة كاملة** — فالـWaterfall (اللي يفترض تجميد المطلوبات أول) **يتأخر جداً**، والبرنامج يوصل **بعد ما تغير سبب شرائه أصلاً**.
...
**AR.** توازن مهم: **plan-driven صح للأنظمة الحرجة** (اللي تحتاج تحليل كامل)، بس **فاشل للأنظمة التجارية السريعة**. هذا يربط بملف 02 (أنظمة حرجة → Waterfall) ويفتح باب Agile.
...
**AR.** رقم قوي من Mall: **40% من المطلوبات تظهر بعد ما يبلش التطوير**. هذا يقتل فكرة "جمّد المطلوبات أول" عملياً.
...
**AR.** تعريف «الخفة» (agility): **لاءمة العملية للمشروع** — احذف أي نشاط مو ضروري، وابتعد عن أي شي يضيّع وقت.
...
**AR.** **Agile = مظلّة (umbrella term)**، مو نموذج واحد. تذكّر: UP نفسه مذكور ضمن عائلة Agile عند Mall — يعني UP وAgile **مو متناقضين**، UP هو نسخة «منضبطة/ثقيلة» من نفس العائلة.
...
**AR.** الاتفاق بين المصدرين: تكرارات **صغيرة (~أسبوعين)**، **تسليم للعميل كل تكرار**، **إشراك العميل**، **توثيق م minimised**. الفرق الطفيف: Mall يضيف **«نخطط/نطوّر/ننشر زيادة واحدة بس كل مرة + ما أكو خطط طويلة المدى»**.
...
**AR.** **هذا يختلف عن time-box حق RAD (ملف 05):** هنا الـtime box = **مدة التكرار**، وتاريخ التسليم **مقدّس لا يتغيّر** — فإذا تأخرت، **يُقلّص functionality مو يُمدّد التاريخ**. (بـRAD كان الـtime-box = الحد الأقصى لكل ميزة). الاثنان يستخدمان نفس المصطلح بسياق مختلف.
...
**AR.** فريق **صغير (5–9)** + **ممثل عميل داخل الفريق** — هذا يفرّق Agile جذرياً عن النماذج الكبيرة (اللي فيها عقود وتفاوض رسمي).
...
**AR.** ثلاث خصائص: (1) المواصفة/التصميم/التنفيذ **متداخلة** (مو متسلسلة) + توثيق م minimised؛ (2) سلسلة إصدارات + **العملاء يقيّمون ويقترحون تغييرات**؛ (3) واجهات تُطوّر بأدوات تفاعلية.
...
**AR.** الفرق الجوهري: Waterfall يقيس التقدّم بـ**المنتجات الموثّقة** (artifacts)؛ Agile بـ**الوظائف المُسلَّمة**. ونقطة ذكية: Agile يستخدم Waterfall **بمقياس صغير بكل تكرار** (يكرّر الدورة كاملة). وإذا أُلغي المشروع منتصفاً: Waterfall **ما يترك شي غير أوراق**؛ Agile **يترك كود شغّال**.
...
**AR.** Agile **مو** «برمجة استكشافية» (chaotic coding) — عنده **عمليات منضبطة والتزام بتصميم رigorous**. التشابه بالتواصل وجهاً لوجه والتوثيق الخفيف فقط.
...
**AR.** **هذا يربط بملف 05:** RAD = نماذج أولية «quick-and-dirty» ثم تُحسّن؛ Agile = **تطوير منهجي لكل ميزة** بدون نماذج أولية. Agile يعرض **شغل مكتمل**؛ RAD يعرض **mock-ups**. (تذكّر: RAD يبني ميزة واحدة بس بـmini-projects متوازية؛ Agile يبني زيادة واحدة متسلسلة بس).
...
**AR.** **الميزة:** اعتماد على المعرفة الضمنية (tacit knowledge) + تواصل غير رسمي = خفّة. **العيوب الثلاثة:** (1) توثيق ضعيف → تشوّش وسوء فهم؛ (2) صعوبة مراجعة القرارات من خبراء خارجيين؛ (3) **المحافظة (maintenance) تصير مشكلة** لما يتفرّق المطوّرون. هذي العيوب الثلاثة تردّد صدى عيوب النماذج المُرمية (ملف 03).

---

## 7. Extreme Programming (XP) — Mall Primary

**[THIN]** — Sommerville names XP in one sentence (*"Probably the best-known agile method is extreme programming (Beck, 1999; Beck, 2000)"*, p.76) and lists it among others, but the **substantive XP treatment is Mall-only**. The same applies to Scrum.
...
**AR.** فكرة XP: **خذ أفضل الممارسات وطبّقها لحد التطرف** (extreme). الفلسفة: *«إذا شي مفيد، ليش ما نستخدمه باستمرار؟»*
...
**AR.** ست ممارسات (Mall سمّاها 6 بس خمسة منها «to the extreme» + integration): 
...
**AR.** الأنشطة الستة: Coding · Testing · Listening (الاستماع للعميل لأنه يملك المعرفة بالمجال) · Designing · Feedback («نظام بعيد عن المستخدمين = مشكلة قادمة») · Simplicity («ابنِ شي بسيط يشتغل اليوم، لا تبنِ شي يأخذ وقتاً وربما لن يُستخدم أبداً»).
...
**AR.** **XP يناسب:** تقنية جديدة/بحث (مطلوبات سريعة التغيّر) + مشاريع صغيرة. **ما يناسب:** مطلوبات مستقرة (استخدم iterative waterfall) + أنظمة حرجة/مهمة بسلامة (استخدم SDLC التقليدي). هذا **يطابق تماماً** ما قاله Sommerville بص74 (plan-driven صح للأنظمة الحرجة).
...
**AR.** خمسة مبادئ (من Sommerville): إشراك العميل · التسليم التزايدي · **الناس لا العملية** · احتضان التغيير · الحفاظ على البساطة. هذي تلخّص فلسفة Agile كلها.
...
**The decision rule applied (per student instruction):** all sources read in full before writing; nothing compressed — this is a compendium. The manifesto is shown in **both** wordings (§4) because Sommerville gives the canonical 2001 four and Mall gives a restated, expanded version; they are presented as a source difference, not merged. Where Mall alone supplies material (agile-vs-other-models, XP mechanics, Scrum roles), it is marked `[THIN]` so a student knows Sommerville will not supply it. Rule 3 (show both) and Rule 6 ([THIN]) honoured. [Source Difference]

---

## 12. Retrieval Set — 16 Items
