...
> **Subject · Week · File:** 04_Advanced_Software_Eng · Week 02 · File 10 of 10 · Master Comparison and Exam Bank
> **Sources:** Consolidation of Files 01–09 (Sommerville Ch.2; Mall Ch.2 §2.2–§2.6 & §8.3.1; Pressman spiral; Agarwal RAD). **No new primary material** — every claim here traces to a prior file and therefore to a source page already anchored there. This file is the index to the series.

---

## WHERE THIS SITS

**File 09 (Choosing a Model)** closed the argument: no model wins, only fits — selection is driven by the product, the team, the customer, and above all by **requirements and risk**.

**This file answers:** *"Everything at once — can I see all ten models side by side, and can I drill every topic back to its retrieval question?"* It is the **consolidation**: one master comparison table, the two recurring axes laid bare, Mall's exercises mapped to the file that answers each, and the **complete retrieval bank** (all 133 questions from Files 01–09) in one place.
...
|:--:|:---|:---|:---|:---|:---|:---|:---|
| 1 | **Build & Fix / Exploratory** | Trivial / single-programmer "toy" work; "comes naturally to first-time programmers" | Poor quality, unmaintainable, expensive for non-trivial; **fails at team scale** (coordination breaks) | None managed | None captured | n/a | File 02 / Mall p.28 |
| 2 | **Classical Waterfall** | Requirements **well understood and unlikely to change radically**; also implicitly used to *document* any software | **No feedback paths**; idealistic; cannot absorb change | Assumes no errors occur | Frozen up front | Large up-front (monolithic delivery) | File 02 / Mall p.73–81; Sommerville p.47–49 |
| 3 | **Iterative Waterfall** | **Well-understood problems**; the most widely used model | Not for **very large** or **high-risk** projects | Feedback paths allow correction, but no risk *analysis* | Frozen up front (with correction) | Monolithic | File 02 / Mall p.83–84; File 09 p.118 |
| 4 | **V-Model** | **Safety-critical / high-reliability**; verification & validation run throughout | **Sequential** (no iteration); rigid | Testing built in from the start | Frozen | Monolithic | File 02 / Mall p.88–89 `[THIN: Sommerville has no V-model]` |
| 5 | **Prototyping** | GUI part; unclear technical solution; requirements/technical **not** understood but **all risks identifiable up front** | Prototype not used the same way as final; pressure to deliver throwaway; degraded structure | **Change avoidance**; handles *known* risks | Discovered via prototype; SRS usually still needed (except GUI) | Lower up-front | File 03 / Sommerville p.62–63; Mall p.91–93 |
| 6 | **Evolutionary** | Large **decomposable** problems; OO; customer accepts incremental delivery | Feature division non-trivial; ad-hoc design; *not* best when requirements are clear (iterative waterfall wins) | **Change tolerance** | **Evolve**, not frozen | Incremental | File 03 / Mall p.97–99 |
| 7 | **Incremental Dev & Delivery** | Large / slow-changing; **most common** approach today; can be plan-driven, agile, or mixed | Process **not visible**; structure **degrades**; not for very large / embedded / critical | **Change tolerance** (later increments absorb change) | Core first; later outlined; *current* increment frozen | Incremental (no large up-front) | File 04 / Sommerville p.49–51, 64–65; Mall p.95–97 |
| 8 | **RAD** | Small/medium; completable in **2–3 months**; requirements **known clearly**; need for speed | Only small/medium; needs **active customer + collocated team**; unsuitable for high technical risk | Minimal (assumes known requirements) | Less formal (automated tools), known up front | Fast | File 05 / Mall p.100–104 `[THIN: Sommerville/Pressman cover RAD only in passing]` |
| 9 | **Spiral** | Technically challenging, large, risks **hard to anticipate at start** (emerge during) | Complex; hard to convince customers; needs risk-assessment expertise; less widely used | **Explicit risk-driven**; meta-model encompassing all others | Evolve per loop; prototype each phase | Refined over loops | File 06 / Sommerville p.65–67; Mall p.114–116; Pressman p.64–68 |
| 10 | **Unified Process (RUP)** | OO; large; wants incremental + iterative but **disciplined** | **Heavy**; not for embedded; needs experienced team | Risk addressed in Elaboration (key risks) | Use-case-driven; elaborated | Phased | File 07 / Mall p.475–476; Sommerville p.67–70 |
...
### 2.2 Lens A — Mall's three selection factors (File 09, p.120)
...
### 2.3 Lens B — Sommerville's critical↔business + plan-driven↔agile axes (File 01/09, p.46)
...
> **No contradiction.** Lens A (three factors) and Lens B (two axes) are **complementary descriptions of the same choice** — requirements-and-risk is the underlying axis; Mall's factors are the concrete knobs. Shown side by side in File 09 §5, not merged.

### 2.4 The narrative in one line per model

Build&Fix (no process) → Waterfall (freeze) → Iterative (feedback) → V (verify throughout) → Prototyping (learn reqs) → Evolutionary (evolve) → Incremental (slice) → RAD (compress) → Spiral (risk-drive) → UP (hybrid-discipline) → Agile (embrace change). Each step is the previous one's answer to *"but requirements change / risk appears / deadline bites."*
...
|:---|:---|:---|
| Q1–Q2, Q5–Q6 | SDLC / process / methodology definitions | **File 01** |
| Q9, Q11, Q12, Q15, Q17, Q51 | Waterfall phases, effort, idealistic critique, phase completeness | **File 02** |
| Q20, Q68 | Build-and-fix description & activities | **File 02** |
| Q21, Q45 | T/F on models; iterative-waterfall shortcomings + which model overcomes each | **File 02** (+ File 09 for the "which model" part) |
| Q25–Q27, Q46 | V-model strengths/weaknesses & suitability | **File 02** |
| Q19, Q39, Q52 | Incremental vs evolutionary; evolutionary model | **File 03** / **File 04** |
| Q27–Q30, Q32, Q34, Q35, Q37, Q41, Q50 | Prototyping vs spiral; spiral meta-model; loops; why spiral may not be prudent | **File 03** / **File 06** |
| Q29, Q32, Q33, Q34, Q38 | Risk handling: prototyping vs spiral; spiral for MIS | **File 06** |
| Q24 (a–i) | "Which model for each application?" (data processing, satellite comm, telephone switch, library automation, cellular-via-satellites, text editor, compiler, OO, GUI) | **File 09** (selection) + Files 02–06 |
| Q40 | Payroll software → which model (experienced team) | **File 09** |
| Q44 | Why final docs written as-if waterfall | **File 09** (also **File 02** §2.2.1) |
| Q47–Q48, Q54, Q59, Q60 | RAD motivation, aspects, advantages vs iterative/evolutionary | **File 05** |
| Q49 | Customer unsure + requirements change frequently → which model | **File 03** / **File 09** |
| Q53, Q55 | Three factors; important factors influencing choice | **File 09** |
| Q56–Q58, Q61–Q64, Q67 | Agile features, XP practices, pairwise programming, cost/time reduction | **File 08** |
| Q65 | Customising existing software → agile vs iterative waterfall | **File 08** |
| Q66 | OO software → which model | **File 07** / **File 09** |
| Q62 | Waterfall/RAD/agile shortcomings | **File 02** / **File 05** / **File 08** |
...
- **This file is a consolidation.** Its authority is entirely inherited from Files 01–09, each of which was built directly from Sommerville, Mall, Pressman, and Agarwal with verbatim anchors. No new interpretation is introduced here that is not already in those files.
- **Counts:** 11 models compared (§1); 2 axes + 3 factors recapped (§2); 30+ exercise clusters mapped (§3); **133 retrieval items** consolidated (§Retrieval Set): File 01 = 12, Files 02–08 = 16 each (112), File 09 = 9.
- **What adds nothing new:** Pressman and Agarwal appear only through Files 05–06; they contribute no unique claim at the consolidation level, so they are not re-cited here except where a prior file already carried them.
- **[THIN] flag (carried forward):** File 05 (RAD) could not be checked against Sommerville at all (0 relevant hits); File 02's V-model and File 03's evolutionary model are Mall-only. These gaps are inherited, not closed, by this file — noted so the student knows where a second source does not exist.
...
### From File 02 — Build & Fix and the Waterfall Family (16 items)
...
### From File 03 — Prototyping and the Evolutionary Model (16 items)
...
### From File 04 — Incremental Development (16 items)
...
### From File 06 — The Spiral Model (16 items)
...
### From File 07 — The Unified Process (16 items)
...
### From File 08 — Agile, XP and Scrum (16 items)
...
> **Answer:** The duration of one iteration; the end date is sacrosanct and if late, functionality is reduced (not the date extended). (RAD's time box = max time per feature.) [Mall p.107; contrasts File 05]
...
### From File 09 — Choosing a Model (9 items)
...
## PAGE ANCHORS — Consolidated (Files 01–09)

**File 01 — SDLC Fundamentals:** Sommerville p.44–46 · Mall p.67–71.
**File 02 — Build & Fix / Waterfall:** Mall p.28, 73–89 · Sommerville p.46–49.
**File 03 — Prototyping / Evolutionary:** Sommerville p.60–65 · Mall p.91–99.
**File 04 — Incremental:** Sommerville p.47, 49–51, 64–65 · Mall p.95–97.
**File 05 — RAD:** Mall p.100–104 · Pressman p.60/76 · Agarwal p.62 (passing). `[THIN: Sommerville 0 hits]`
**File 06 — Spiral:** Sommerville p.65–67 · Mall p.114–116 · Pressman p.64–68.
**File 07 — Unified Process:** Mall p.475–476 · Sommerville p.67–70.
**File 08 — Agile / XP / Scrum:** Sommerville p.74–77 · Mall p.105–114.
**File 09 — Choosing a Model:** Mall p.118–121, 130–131 · Sommerville p.46.
...
**End of File 10 — end of the Week 02 series (10 of 10).**
