...
- [ ] **`01_Cyber_Security` daily quiz** — postponed; now covers two booklets. Week 02 Risk booklet 2 has been delivered and is staged in `02_Raw_Materials`; exact quiz date not confirmed. **Still needed: the illustrated booklet (the delivered DOCX has zero images), the student's lecture notes on the two equations, or the chapter PDF via library access.**
