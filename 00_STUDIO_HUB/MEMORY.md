...
- **Added 2026-09-22 (late):** W02 FUZZY BOOKLET DELIVERED — `05_Soft_Computing/03_Study_Notes/w02-fuzzy-logic-systems-booklet.{html,pdf}` (29pp = cover + 28 sections), commit `a8cd34e`, pushed. Built by `90_Shared_Toolbox/tools/build_w02_fuzzy_booklet.py`; PDF via headless Chromium at `~/AppData/Local/ms-playwright/chromium-1243/chrome-win64/chrome.exe` with `--virtual-time-budget=40000` (MathJax needs it). Both "still open" items from the analysis (§6.2: pp.47–55 visuals, pp.62–66 examples) are now closed and source-verified against page images (p59 city grades, p60 formal def, p65 Case 1, p66 Case 2).
...
- **Added 2026-09-23 (extraction rule — read before parsing any DOCX):** **a marked-up booklet is the professor telling you the exam scope.** Before writing any study note from a DOCX, enumerate its formatting layer: `xml.count("<w:highlight")`, `<w:color`, `<w:shd`, `<w:pict`/`v:shape`/`wps:txbx`, `<w:tbl>`. Then walk **runs**, not paragraphs — formatting lives on the run. I first read only `<w:t>` runs and concluded the Cyber W02 Risk file was plain text, when it in fact carried **30 highlight runs + 27 coloured runs** — the doctor's own emphasis map, thrown away. Also check `word/media` (empty = the document has **no images at all**, even when the text says "Fig. 3.1"), and `<w:sym>` (Word encodes ×, ÷, → as symbol elements that a `<w:t>`-only regex silently drops — this is why the Risk formulas lost their operators).

- **Added 2026-09-23 (Dr. Huda Cyber W02 emphasis, objective):** yellow highlight = the 5 risk-mitigation strategies, COBIT/COSO/FAIR/OCTAVE, ISO/IEC 27002 "14 categories", OCTAVE "three phases". Red highlight = 3 OCTAVE phases, 4 PDCA elements, physical-security headings, 4 authentication factors. Coloured text = 4 threat groups, countermeasure matching, equation letters `S`/`F`/`K`. **Student-verified "not important": both NIST blocks** ("Develop a risk-management program", "Use NIST security controls"). Lecture stopped at Authentication. Exam-focus sheet: `01_Cyber_Security/03_Study_Notes/Week_02_Risk_Exam_Focus.md`.

- **Added 2026-09-23 (Academic PDF Publishing Engine — pdf_exporter.py):** Permanent resolution to the vault-wide agent PDF generation problem. Canonical tool: `90_Shared_Toolbox/tools/pdf_exporter.py` + batch wrapper `export-pdf.bat`. Driven by local Playwright Chromium (`~/AppData/Local/ms-playwright/chromium-1243/...`) using DirectWrite / HarfBuzz. Four templates: `study_pack` (default), `booklet`, `exam_sheet`, `glossary`. Zero SaaS, zero daemons, 100% offline. Full guide: `00_STUDIO_HUB/guides/PDF_PUBLISHING_SOP.md`.
  - **Lesson 1 (Windows File Locking):** If a student has the PDF open in Adobe Acrobat or Edge, Windows locks the file exclusively (`PermissionError: [Errno 13]`). The exporter catches this and automatically writes to `<filename>_new.pdf` instead of crashing.
